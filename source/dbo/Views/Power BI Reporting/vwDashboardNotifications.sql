...
CREATE VIEW [dbo].[vwDashboardNotifications]
	...
	SELECT DISTINCT
		...
		LEFT OUTER JOIN [dbo].[vwAlert] anyAlert ON anyAlert.NotificationId = cd.NotificationId AND anyAlert.AlertStatus = 'Open'
	...
	--case has no outcome or case has an open alert
	AND (cd.LastRecordedTreatmentOutcomeDescriptive IN ('No outcome recorded', 'Not evaluated - still on treatment') 
	OR anyAlert.NotificationId IS NOT NULL)