CREATE VIEW [dbo].[vwUnmatchedSpecimens]
	...
	SELECT DISTINCT ls.[ReferenceLaboratoryNumber]
			...
			,FIRST_VALUE(p.[Name]) OVER (PARTITION BY us.ReferenceLaboratoryNumber ORDER BY p.[Name] DESC) AS Region
		...
		LEFT JOIN [$(ETS)].dbo.LaboratoryHospitalMapping lhm on lhm.SourceLabName = ls.LaboratoryName AND lhm.AuditDelete is NULL
