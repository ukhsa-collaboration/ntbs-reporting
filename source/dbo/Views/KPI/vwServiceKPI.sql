...
AS
	SELECT tbs.TB_Service_Code, tbs.TB_Service_Name, map.PHEC_Code, phec.PHEC_Name,
		...
		FROM [$(NTBS_Geography_Staging)].[dbo].[TB_Service] tbs
		LEFT JOIN [$(NTBS_Geography_Staging)].[dbo].[TB_Service_to_PHEC] map
			ON tbs.TB_Service_Code = map.TB_Service_Code
		LEFT JOIN [$(NTBS_Geography_Staging)].[dbo].[PHEC] phec
