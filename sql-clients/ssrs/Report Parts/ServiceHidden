<?xml version="1.0" encoding="utf-8"?>
<ComponentItem Name="ServiceHidden" xmlns="http://schemas.microsoft.com/sqlserver/reporting/2010/01/componentdefinition" xmlns:rdl="http://schemas.microsoft.com/sqlserver/reporting/2016/01/reportdefinition" xmlns:rd="http://schemas.microsoft.com/SQLServer/reporting/reportdesigner">
  <Properties>
    <Property Name="Type">ReportParameter</Property>
    <Property Name="ThumbnailSource"></Property>
    <Property Name="ThumbnailMimeType">image/png</Property>
  </Properties>
  <RdlFragment>
    <rdl:Report>
      <rdl:AutoRefresh>0</rdl:AutoRefresh>
      <rdl:DataSources>
        <rdl:DataSource Name="PHE">
          <rdl:ConnectionProperties>
            <rdl:DataProvider>SQL</rdl:DataProvider>
            <rdl:ConnectString>Data Source=sqlnisntbscol02;Initial Catalog=NTBS_R1_Reporting_Staging</rdl:ConnectString>
            <rdl:IntegratedSecurity>true</rdl:IntegratedSecurity>
            <rdl:Prompt>Enter a user name and password to access the data source:</rdl:Prompt>
          </rdl:ConnectionProperties>
          <rd:SecurityType>Integrated</rd:SecurityType>
          <rd:DataSourceID>c4640378-9393-438a-a618-034c268737e7</rd:DataSourceID>
        </rdl:DataSource>
      </rdl:DataSources>
      <rdl:DataSets>
        <rdl:DataSet Name="ServicesHiddenShared">
          <rdl:Query>
            <rdl:DataSourceName>PHE</rdl:DataSourceName>
            <rdl:CommandType>StoredProcedure</rdl:CommandType>
            <rdl:CommandText>uspService</rdl:CommandText>
          </rdl:Query>
          <rdl:Fields>
            <rdl:Field Name="TB_Service_Name">
              <rdl:DataField>TB_Service_Name</rdl:DataField>
              <rd:TypeName>System.String</rd:TypeName>
            </rdl:Field>
          </rdl:Fields>
        </rdl:DataSet>
      </rdl:DataSets>
      <rdl:ReportSections>
        <rdl:ReportSection>
          <rdl:Body>
            <rdl:Height>0cm</rdl:Height>
            <rdl:Style />
          </rdl:Body>
          <rdl:Width>0cm</rdl:Width>
          <rdl:Page>
            <rdl:Style />
          </rdl:Page>
        </rdl:ReportSection>
      </rdl:ReportSections>
      <rdl:ReportParameters>
        <rdl:ReportParameter Name="ServiceHidden">
          <rdl:DataType>String</rdl:DataType>
          <rdl:DefaultValue>
            <rdl:DataSetReference>
              <rdl:DataSetName>ServicesHiddenShared</rdl:DataSetName>
              <rdl:ValueField>TB_Service_Name</rdl:ValueField>
            </rdl:DataSetReference>
          </rdl:DefaultValue>
          <rdl:Prompt>ServiceHidden</rdl:Prompt>
          <rdl:Hidden>true</rdl:Hidden>
          <rdl:ValidValues>
            <rdl:DataSetReference>
              <rdl:DataSetName>ServicesHiddenShared</rdl:DataSetName>
              <rdl:ValueField>TB_Service_Name</rdl:ValueField>
              <rdl:LabelField>TB_Service_Name</rdl:LabelField>
            </rdl:DataSetReference>
          </rdl:ValidValues>
          <rdl:MultiValue>true</rdl:MultiValue>
          <ComponentMetadata xmlns="http://schemas.microsoft.com/sqlserver/reporting/2010/01/componentdefinition">
            <ComponentId>563d673e-d788-4f71-ac18-cedf98b6fe58</ComponentId>
          </ComponentMetadata>
        </rdl:ReportParameter>
      </rdl:ReportParameters>
      <rdl:ReportParametersLayout>
        <rdl:GridLayoutDefinition>
          <rdl:NumberOfColumns>4</rdl:NumberOfColumns>
          <rdl:NumberOfRows>2</rdl:NumberOfRows>
        </rdl:GridLayoutDefinition>
      </rdl:ReportParametersLayout>
      <rd:ReportUnitType>Invalid</rd:ReportUnitType>
      <rd:ReportID>1eba7b96-8e1e-4b59-a3fb-32ab08c33232</rd:ReportID>
    </rdl:Report>
  </RdlFragment>
</ComponentItem>