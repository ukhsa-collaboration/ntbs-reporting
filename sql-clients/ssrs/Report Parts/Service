<?xml version="1.0" encoding="utf-8"?>
<ComponentItem Name="Service" xmlns="http://schemas.microsoft.com/sqlserver/reporting/2010/01/componentdefinition" xmlns:rdl="http://schemas.microsoft.com/sqlserver/reporting/2016/01/reportdefinition" xmlns:rd="http://schemas.microsoft.com/SQLServer/reporting/reportdesigner">
  <Properties>
    <Property Name="Type">ReportParameter</Property>
    <Property Name="ThumbnailSource"></Property>
    <Property Name="ThumbnailMimeType">image/png</Property>
  </Properties>
  <RdlFragment>
    <rdl:Report>
      <rdl:AutoRefresh>0</rdl:AutoRefresh>
      <rdl:DataSources>
        <rdl:DataSource Name="PHE">
          <rdl:ConnectionProperties>
            <rdl:DataProvider>SQL</rdl:DataProvider>
            <rdl:ConnectString>Data Source=sqlnisntbscol02;Initial Catalog=NTBS_R1_Reporting_Staging</rdl:ConnectString>
            <rdl:IntegratedSecurity>true</rdl:IntegratedSecurity>
            <rdl:Prompt>Enter a user name and password to access the data source:</rdl:Prompt>
          </rdl:ConnectionProperties>
          <rd:SecurityType>Integrated</rd:SecurityType>
          <rd:DataSourceID>c4640378-9393-438a-a618-034c268737e7</rd:DataSourceID>
        </rdl:DataSource>
      </rdl:DataSources>
      <rdl:DataSets>
        <rdl:DataSet Name="RegionShared">
          <rdl:Query>
            <rdl:DataSourceName>PHE</rdl:DataSourceName>
            <rdl:QueryParameters>
              <rdl:QueryParameter Name="@Option">
                <rdl:Value>ADD_UNKNOWN</rdl:Value>
              </rdl:QueryParameter>
            </rdl:QueryParameters>
            <rdl:CommandType>StoredProcedure</rdl:CommandType>
            <rdl:CommandText>uspServicePhec</rdl:CommandText>
          </rdl:Query>
          <rdl:Fields>
            <rdl:Field Name="PhecName">
              <rdl:DataField>PhecName</rdl:DataField>
              <rd:TypeName>System.String</rd:TypeName>
            </rdl:Field>
          </rdl:Fields>
        </rdl:DataSet>
        <rdl:DataSet Name="ServiceShared">
          <rdl:Query>
            <rdl:DataSourceName>PHE</rdl:DataSourceName>
            <rdl:QueryParameters>
              <rdl:QueryParameter Name="@Region">
                <rdl:Value>=Parameters!Region.Value</rdl:Value>
              </rdl:QueryParameter>
            </rdl:QueryParameters>
            <rdl:CommandText>select Serviceid,TB_Service_Name from TB_Service 
where PhecName = @Region
</rdl:CommandText>
          </rdl:Query>
          <rdl:Fields>
            <rdl:Field Name="Serviceid">
              <rdl:DataField>Serviceid</rdl:DataField>
              <rd:TypeName>System.Int32</rd:TypeName>
            </rdl:Field>
            <rdl:Field Name="TB_Service_Name">
              <rdl:DataField>TB_Service_Name</rdl:DataField>
              <rd:TypeName>System.String</rd:TypeName>
            </rdl:Field>
          </rdl:Fields>
        </rdl:DataSet>
      </rdl:DataSets>
      <rdl:ReportSections>
        <rdl:ReportSection>
          <rdl:Body>
            <rdl:Height>0cm</rdl:Height>
            <rdl:Style />
          </rdl:Body>
          <rdl:Width>0cm</rdl:Width>
          <rdl:Page>
            <rdl:Style />
          </rdl:Page>
        </rdl:ReportSection>
      </rdl:ReportSections>
      <rdl:ReportParameters>
        <rdl:ReportParameter Name="Service">
          <rdl:DataType>String</rdl:DataType>
          <rdl:DefaultValue>
            <rdl:DataSetReference>
              <rdl:DataSetName>ServiceShared</rdl:DataSetName>
              <rdl:ValueField>Serviceid</rdl:ValueField>
            </rdl:DataSetReference>
          </rdl:DefaultValue>
          <rdl:Prompt>*Service</rdl:Prompt>
          <rdl:ValidValues>
            <rdl:DataSetReference>
              <rdl:DataSetName>ServiceShared</rdl:DataSetName>
              <rdl:ValueField>Serviceid</rdl:ValueField>
              <rdl:LabelField>TB_Service_Name</rdl:LabelField>
            </rdl:DataSetReference>
          </rdl:ValidValues>
          <rdl:MultiValue>true</rdl:MultiValue>
          <ComponentMetadata xmlns="http://schemas.microsoft.com/sqlserver/reporting/2010/01/componentdefinition">
            <ComponentId>dfdb2194-5965-44ae-a3a4-b153af9b5ae8</ComponentId>
          </ComponentMetadata>
        </rdl:ReportParameter>
        <rdl:ReportParameter Name="Region">
          <rdl:DataType>String</rdl:DataType>
          <rdl:Prompt>*Region</rdl:Prompt>
          <rdl:ValidValues>
            <rdl:DataSetReference>
              <rdl:DataSetName>RegionShared</rdl:DataSetName>
              <rdl:ValueField>PhecName</rdl:ValueField>
              <rdl:LabelField>PhecName</rdl:LabelField>
            </rdl:DataSetReference>
          </rdl:ValidValues>
          <ComponentMetadata xmlns="http://schemas.microsoft.com/sqlserver/reporting/2010/01/componentdefinition">
            <ComponentId>cb9b200c-9939-4412-921f-1746f306fe5c</ComponentId>
            <SourcePath>/Report Parts/Region</SourcePath>
            <SyncDate>2019-10-15T10:14:37.5165856+01:00</SyncDate>
          </ComponentMetadata>
        </rdl:ReportParameter>
      </rdl:ReportParameters>
      <rdl:ReportParametersLayout>
        <rdl:GridLayoutDefinition>
          <rdl:NumberOfColumns>4</rdl:NumberOfColumns>
          <rdl:NumberOfRows>2</rdl:NumberOfRows>
        </rdl:GridLayoutDefinition>
      </rdl:ReportParametersLayout>
      <rd:ReportUnitType>Invalid</rd:ReportUnitType>
      <rd:ReportID>d7557363-8f47-4f8d-b33e-1d37f85fafae</rd:ReportID>
    </rdl:Report>
  </RdlFragment>
</ComponentItem>